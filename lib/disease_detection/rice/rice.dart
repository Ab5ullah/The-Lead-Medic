...
import 'rice_bacterial_leaf_blight.dart';
import 'rice_brown_spot.dart';
import 'rice_hispa.dart';
import 'rice_leaf_blast.dart';
import 'rice_tungro.dart';
...
class _RicePageState extends State<RicePage> {
  ...
  Map<String, String> _diseaseDescriptions = {
    'rice_bacterial_leaf_blight':
        'Rice bacterial leaf blight is a major disease affecting rice, causing water-soaked lesions and blighting of leaves. It can lead to significant yield losses if not managed properly.',
    'rice_brown_spot':
        'Rice brown spot is a fungal disease that causes small, dark brown spots on leaves. While it may not be highly damaging, severe infections can reduce the quality and yield of rice.',
    'rice_healthy':
        'The rice crop is healthy. However, proper water management and disease monitoring are essential to ensure good crop performance.',
    'rice_hispa':
        'Rice hispa is an insect pest that feeds on rice leaves, leading to characteristic long, whitish marks on the leaves. Regular scouting and insecticide application are important to manage this pest.',
    'rice_leaf_blast':
        'Rice leaf blast is a fungal disease that causes lesions on leaves, panicles, and nodes, leading to significant yield losses. Early detection and resistant varieties are crucial for effective management.',
    'rice_tungro':
        'Rice tungro is a viral disease transmitted by insect vectors, causing yellowing and stunting of rice plants. Integrated pest management strategies and use of resistant varieties are recommended for control.',
  };
  ...
  Future<void> _uploadAndPredict() async {
    if (_images.length == _maxImages) {
      final uri = Uri.parse('http://192.168.1.5:8000/predict/rice');
  ...
  final List<String> tips = [
    'Capture a clear image of the Rice leaf.',
  ...
  Widget _getDiseasePage(String formattedClassName) {
    switch (formattedClassName) {
      case 'rice_bacterial_leaf_blight':
        return RiceBacterialLeafBlightPage();
      case 'rice_brown_spot':
        return RiceBrownSpotPage();
      case 'rice_hispa':
        return RiceHispaPage();
      case 'rice_leaf_blast':
        return RiceLeafBlastPage();
      case 'rice_tungro':
        return RiceTungroPage();
      default:
