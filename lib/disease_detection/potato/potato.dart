...
import 'potato_early_blight.dart';
import 'potato_healthy.dart';
import 'potato_late_blight.dart';
...
class _PotatoPageState extends State<PotatoPage> {
  ...
  Future<void> _uploadAndPredict() async {
    if (_images.length == _maxImages) {
      final uri = Uri.parse('http://192.168.1.5:8000/predict/potato');
  ...
  final List<String> tips = [
    'Capture a clear image of the Potato leaf.',
  ...
  Widget _getDiseasePage(String formattedClassName) {
    switch (formattedClassName) {
      case 'potato_early_blight':
        return PotatoEarlyBlightPage();
      case 'potato_healthy':
        return PotatoHealthyPage();
      case 'potato_late_blight':
        return PotatoLateBlightPage();
