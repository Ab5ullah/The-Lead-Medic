...
import 'sugarcane_bacterial_blight.dart';
import 'sugarcane_red_rot.dart';
import 'sugarcane_red_rust.dart';

class SugarcanePage extends StatefulWidget {
  @override
  _SugarcanePageState createState() => _SugarcanePageState();
}

class _SugarcanePageState extends State<SugarcanePage> {
  ...
  Map<String, String> _diseaseDescriptions = {
    'sugarcane_bacterial_blight':
        'Sugarcane bacterial blight is a serious disease that affects sugarcane crops, causing water-soaked lesions on leaves and reducing sucrose content. Proper management practices and resistant varieties are crucial to control its spread and minimize losses.',
    'sugarcane_healthy':
        'The sugarcane crops is healthy. Regular monitoring and good agricultural practices are necessary to ensure the successful growth and yield of sugarcane crops.',
    'sugarcane_red_rot':
        'Sugarcane red rot is a fungal disease that affects the internal tissues of sugarcane, causing red discoloration and rotting. It can lead to severe yield losses, and infected leaves should be removed to prevent further spread.',
    'sugarcane_red_rust':
        'Sugarcane red rust is a fungal disease characterized by reddish-brown powdery pustules on leaves. While it may not cause significant damage in the early stages, heavy infections can lead to yield reduction. Timely application of fungicides and resistant varieties can help manage this disease.',
  };
  ...
  Future<void> _uploadAndPredict() async {
    if (_images.length == _maxImages) {
      final uri = Uri.parse('http://192.168.1.5:8080/predict/sugarcane');
  ...
  final List<String> tips = [
    'Capture a clear image of the Sugarcane leaf.',
  ...
  Widget _getDiseasePage(String formattedClassName) {
    switch (formattedClassName) {
      case 'sugarcane_bacterial_blight':
        return SugarcaneBacterialBlightPage();
      case 'sugarcane_red_rot':
        return SugarcaneRedRotPage();
      case 'sugarcane_red_rust':
        return SugarcaneRedRustPage();
      default:
