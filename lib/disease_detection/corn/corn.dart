...
import 'corn_blight.dart';
import 'corn_common_rust.dart';
import 'corn_gray_leaf_spot.dart';

class CornPage extends StatefulWidget {
  @override
  _CornPageState createState() => _CornPageState();
}

class _CornPageState extends State<CornPage> {
  ...
  Future<void> _chooseImage(ImageSource source) async {
    ...
    if (permissions) {
      ...
      if (pickedFile != null) {
        ...
        // Optional: Check if exactly 3 images are selected
        if (_images.length < _maxImages) {
          _showUploadThreeImagesDialog();
        }
      } else {
        // Show dialog or handle case where no image was selected
  ...
  Future<void> _uploadAndPredict() async {
    if (_images.length == _maxImages) {
      final uri = Uri.parse('http://192.168.43.30:8080/predict/corn');
  ...
  final List<String> tips = [
    'Capture a clear image of the Corn leaf.',
  ...
  Widget _getDiseasePage(String formattedClassName) {
    switch (formattedClassName) {
      case 'corn_blight':
        return CornBlightPage();
      case 'corn_common_rust':
        return CornCommonRustPage();
      case 'corn_gray_leaf_spot':
        return CornGrayLeafSpotPage();
      default:
