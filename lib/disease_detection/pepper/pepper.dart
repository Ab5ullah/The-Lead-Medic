...
import 'pepper_bell_bacterial_spot.dart';
import 'pepper_bell_healthy.dart';

class PepperPage extends StatefulWidget {
  @override
  _PepperPageState createState() => _PepperPageState();
}
...
class _PepperPageState extends State<PepperPage> {
  ...
  Future<void> _uploadAndPredict() async {
    if (_images.length == _maxImages) {
      final uri = Uri.parse('http://192.168.1.2:8000/predict/pepper');
  ...
  final List<String> tips = [
    'Capture a clear image of the Pepper leaf.',
  ...
  Widget _getDiseasePage(String formattedClassName) {
    switch (formattedClassName) {
      case 'pepper_bacterial_spot':
        return PepperBacterialSpotPage();
      case 'pepper_healthy':
        return PepperHealthyPage();
