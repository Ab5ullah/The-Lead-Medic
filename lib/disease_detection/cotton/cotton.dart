...
import 'cottonaphids.dart';
import 'cottonarmyworm.dart';
import 'cottonbacterialblight.dart';
import 'cottonpowderymildew.dart';
import 'cottontargetspot.dart';
...
class _CottonPageState extends State<CottonPage> {
  ...
  Future<void> _uploadAndPredict() async {
    if (_images.length == _maxImages) {
      final uri = Uri.parse('http://192.168.1.2:8000/predict/cotton');
  ...
  final List<String> tips = [
    'Capture a clear image of the Cotton leaf.',
  ...
  Widget _getDiseasePage(String formattedClassName) {
    switch (formattedClassName) {
      case 'cotton_aphids':
        return CottonAphidsPage();
      case 'cotton_army_worm':
        return CottonArmyWormPage();
      case 'cotton_bacterial_blight':
        return CottonBacterialBlightPage();
      case 'cotton_powdery_mildew':
        return CottonPowderyMildewPage();
      case 'cotton_target_spot':
        return CottonTargetSpotPage();
      default:
