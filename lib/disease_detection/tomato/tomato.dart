...
import 'tomato_bacterial_spot.dart';
import 'tomato_early_blight.dart';
import 'tomato_healthy.dart';
import 'tomato_late_blight.dart';
import 'tomato_leaf_Mold.dart';
import 'tomato_septoria_leaf_spot.dart';
import 'tomato_spider_mite.dart';
import 'tomato_target_spot.dart';
import 'tomato_tomato_mosaic_virus.dart';
import 'tomato_yellow_leaf_curl_virus.dart';

class TomatoPage extends StatefulWidget {
  @override
  _TomatoPageState createState() => _TomatoPageState();
}
...
class _TomatoPageState extends State<TomatoPage> {
  ...
  Map<String, String> _diseaseDescriptions = {
    'corn_blight':
        'Corn blight is a fungal disease that affects corn plants, causing lesions on the leaves and reducing yield. It is important to take precautionary measures to prevent its spread and minimize its impact on crops.',
    'corn_common_rust':
        'Corn common rust is a fungal disease characterized by orange or rust-colored pustules on the leaves. While it may not always cause significant damage, severe infections can lead to reduced photosynthesis and yield losses.',
    'corn_gray_leaf_spot':
        'Corn gray leaf spot is caused by a fungus that produces small, rectangular lesions with gray centers and dark borders on corn leaves. The disease can spread rapidly, leading to defoliation and yield reduction.',
    'corn_healthy':
        'The corn plant is healthy. However, it is still essential to monitor the crop regularly and implement proper agricultural practices to ensure optimal growth and yield.',
  };
  ...
  Future<void> _uploadAndPredict() async {
    if (_images.length == _maxImages) {
      final uri = Uri.parse('http://192.168.1.5:8080/predict/tomato');
  ...
  final List<String> tips = [
    'Capture a clear image of the Tomato leaf.',
  ...
  Widget _getDiseasePage(String formattedClassName) {
    switch (formattedClassName) {
      case 'tomato_bacterial_spot':
        return TomatoBacterialSpotPage();
      case 'tomato_early_blight':
        return TomatoEarlyBlightPage();
      case 'tomato_healthy':
        return TomatoHealthyPage();
      case 'tomato_late_blight':
        return TomatoLateBlightPage();
      case 'tomato_leaf_mold':
        return TomatoLeafMoldPage();
      case 'tomato_septoria_leaf_spot':
        return TomatoSeptoriaLeafPage();
      case 'tomato_spider_mites':
        return TomatoSpiderMitesPage();
      case 'tomato_target_spot':
        return TomatoTargetSpotPage();
      case 'tomato_mosaic_virus':
        return TomatoMosaicVirusPage();
      case 'tomato_yellow_leaf_curl_virus':
        return TomatoYellowLeafCurlVirusPage();
