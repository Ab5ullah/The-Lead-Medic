...
import 'cherry_healthy.dart';
import 'cherry_powdery_mildew.dart';

class CherryPage extends StatefulWidget {
  @override
  _CherryPageState createState() => _CherryPageState();
}

class _CherryPageState extends State<CherryPage> {
  ...
  Future<void> _uploadAndPredict() async {
    if (_images.length == _maxImages) {
      final uri = Uri.parse('http://192.168.43.30:8080/predict/cherry');
  ...
  final List<String> tips = [
    'Capture a clear image of the Cherry leaf.',
  ...
  Widget _getDiseasePage(String formattedClassName) {
    switch (formattedClassName) {
      case 'cherry_healthy':
        return CherryHealthyPage();
      case 'cherry_powdery_mildew':
        return CherryPowderyMildewPage();
