...
import 'wheat_brown_rust.dart';
import 'wheat_septoria.dart';
import 'wheat_yellow_rust.dart';

class WheatPage extends StatefulWidget {
  @override
  _WheatPageState createState() => _WheatPageState();
}
...
class _WheatPageState extends State<WheatPage> {
  ...
  Map<String, String> _diseaseDescriptions = {
    // Wheat diseases
    'wheat_brown_rust':
        'Wheat brown rust is a fungal disease characterized by brown pustules on leaves. It can lead to significant yield losses if not managed through timely fungicide application and resistant cultivars.',
    'wheat_healthy':
        'The wheat crop is healthy. Adequate fertilization and proper disease management are essential for optimal wheat yield and quality.',
    'wheat_septoria':
        'Wheat septoria is a fungal disease that causes small, dark lesions with yellow halos on wheat leaves. It can reduce photosynthesis and impact grain yield if not managed properly.',
    'wheat_yellow_rust':
        'Wheat yellow rust is a fungal disease causing yellow pustules on leaves. Timely application of fungicides and use of resistant varieties are important for controlling this disease.',
  };
  ...
  Future<void> _uploadAndPredict() async {
    if (_images.length == _maxImages) {
      final uri = Uri.parse('http://192.168.1.2:8000/predict/wheat');
  ...
  final List<String> tips = [
    'Capture a clear image of the Wheat leaf.',
  ...
  Widget _getDiseasePage(String formattedClassName) {
    switch (formattedClassName) {
      case 'wheat_brown_rust':
        return WheatBrownRustPage();
      case 'wheat_yellow_rust':
        return WheatYellowRustPage();
      case 'wheat_septoria':
        return WheatSeptoriaPage();
      default:
