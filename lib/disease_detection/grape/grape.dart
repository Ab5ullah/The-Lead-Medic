...
import 'grape_black_rot.dart';
import 'grape_black_measles.dart';
import 'grape_healthy.dart';
import 'grape_leaf_blight.dart';
...
class _GrapePageState extends State<GrapePage> {
  ...
  Future<void> _uploadAndPredict() async {
    if (_images.length == _maxImages) {
      final uri = Uri.parse('http://192.168.1.2:8000/predict/grape');
  ...
  final List<String> tips = [
    'Capture a clear image of the Grape leaf.',
  ...
  Widget _getDiseasePage(String formattedClassName) {
    switch (formattedClassName) {
      case 'grape_black_rot':
        return GrapeBlackRotPage();
      case 'grape_black_measles':
        return GrapeBlackMeaslesPage();
      case 'grape_healthy':
        return GrapeHealthyPage();
      case 'grape_leaf_blight':
        return GrapeLeafBlightPage();
